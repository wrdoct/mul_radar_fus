...
NNDA::NNDA() {
	...
	radar1_measureNum = 0;//雷达1目标个数 / 航迹个数
	radar2_measureNum = 0;//雷达2目标个数 / 航迹个数
		
	radar1OverlapPoints(0, 0);//雷达1重叠区域的位置	
	radar1OverlapVels(0, 0);//雷达1重叠区域的速度
	radar1OverlapRoadId = 0;//雷达1重叠区域的车道号

	radar2OverlapPoints(0, 0);//雷达2重叠区域的位置
	radar2OverlapVels(0, 0);//雷达2重叠区域的速度
	radar2OverlapRoadId = 0;//雷达2重叠区域的车道号
...
void NNDA::TransMatrix(Eigen::Vector2d &radarData, vector<MatrixXd> &tmpData) {
	...
	tmp(0, 0) = radarData[0]; //第一行为x值(单个目标)
	tmp(1, 0) = radarData[1]; //第二行为y值(单个目标)
...
void NNDA::Fusion(vector<RadarInfo_t> &radar1_OverlapStatus, vector<RadarInfo_t> &radar2_OverlapStatus, vector<RadarInfo_t> &mul_radarFusion) {
	radar1_measureNum = radar1_OverlapStatus.size(); 
	radar2_measureNum = radar2_OverlapStatus.size(); 
	//cout << "typeid(radar2_OverlapStatus).name() : " << typeid(radar2_OverlapStatus).name() << endl; //

	for (int i = 0; i < radar1_measureNum; i++) {
		GetDimensionalData(radar1_OverlapStatus[i], radar1OverlapPoints, radar1OverlapVels, radar1OverlapRoadId);
		/*
		...
		*/
		...
		for (int j = 0; j < radar2_measureNum; j++) {				
			GetDimensionalData(radar2_OverlapStatus[j], radar2OverlapPoints, radar2OverlapVels, radar2OverlapRoadId);		
			Eigen::Vector2d overlap_radartoshare_pos;//重叠区域坐标转换结果			
			...
			//执行nnda的操作
			MatrixXd d = radar1_tmpDataPos.at(0).col(0) - radar2_tmpDataPos.at(0).col(0); //雷达1的每列减去雷达2的每列（单个目标）
			cout << "d = " << d << endl;
